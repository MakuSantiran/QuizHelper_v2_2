...
function generate4Questions(QuestionArray, GROUP_ANSWERS, QuestionOrAnswer){
	...
	//mathRandom(min, max)
	var indexlocation = grouped_list.indexOf(QuestionArray.group);
	...
	if (remix3){
		...
		for (i = 0; i<QuestionArray.answer.length; i++){
...
function askquestion(array){
	...
	if ((remix == 4 && array.picture == "img/") && (checkDontInclude(array.group,DontIncludeFromRemix2) == false)){
		answers = [];
		WhatToAsk = "question";
